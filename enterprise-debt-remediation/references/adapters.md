...
核心规则只认 `audit`、`plan`、`fix`。平台入口只做路由，不复制完整还债逻辑。

## Codex

推荐安装两个技能：

- `$hz`：极简入口，适合 `/hz audit`、`/hz plan`、`/hz fix`
- `$enterprise-debt-remediation`：完整审计和整改流程

`$hz` 只返回 `findings`、`next_step`、`blockers`。需要完整台账时切到 `$enterprise-debt-remediation`。
...
- mode: audit, plan, or fix; default audit
...
- 平台不支持 slash command 时，用自然语言触发同样的三个模式。
